	...
	<view class="container">
		<view class="page-body">
			<view class='wrapper'>
				...
				<editor id="editor" class="ql-container" placeholder="请您详细描述所求助的具体信息……" showImgSize showImgToolbar showImgResize
				 @statuschange="onStatusChange" :read-only="readOnly" @ready="onEditorReady"
				 @focus="onFocus" @blur="onBlur" @input="editorChange">
				</editor>
				...
				<div class="video-box">
				      <!-- 预览 -->
				      <div class="video-prev-box" v-if='videoUrl.length > 0'>
				      ...
				      <!-- 上传 -->
				      <div class="video-func-box" v-else @click='uploadVideo'>
				        <div class="upload-lines">
				          <input type="file" name="video" class="videofile" accept="video/mp4" id="videofile">
				          <div class="upload-lines-x"></div>
				          <div class="upload-lines-y"></div>
				        </div>
		...
		methods: {
			...
			nextStep(){
				...
				this.editorCtx.getContents({
					success: (res) => {
						let { html, text } = res
						if(text.length < 20) return uni.showToast({title:'请详细的说明您要发布的内容', icon:'none'})
						
						if(text.length > 800) return uni.showToast({title:'内容超出字数限制', icon:'none'})
						
						uni.setStorageSync('H5_FORHELP', 
							JSON.stringify({'data':
								JSON.stringify({content: html, image_url: this.imgs.substring(0, this.imgs.length-1), video_url: this.videoUrl})
							})
						)
						
						// console.log('h', uni.getStorageSync('H5_FORHELP'))
						//跳转 H5 求助第三步
						window.location.href = 'https://www.zhuanjuan.net/h5/index.html#/pages/forhelp_third/index'
					}
				})
			}
		},
